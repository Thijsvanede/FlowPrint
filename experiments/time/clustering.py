...
if __name__ == "__main__":
    ...
    for size in [1, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9]:
        for random_seed in range(10):
            ...
            time_end   = time.time()
            # Print total time
            print("FIT    : size={:12}({:12}), rs={:2}, time={} seconds".format(size, f.shape[0], random_seed, time_end-time_start))
            ...
            time_end   = time.time()
            # Print total time
            print("PREDICT: size={:12}({:12}), rs={:2}, time={} seconds".format(size, f.shape[0], random_seed, time_end-time_start))
